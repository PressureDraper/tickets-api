...
if __name__ == "__main__":
    #---Connection---
    database = mysql.connector.connect(host = "10.30.0.7", user = "server7", passwd = "Siscem1037", database = "bd_cae")

    database2 = mysql.connector.connect(host = "127.0.0.1", user = "root", passwd = "siscae1035", database = "boletas")
    ...
    cursor.execute("SELECT c.id_residente, c.grado, c.ciclo ,r.matricula FROM ced_per_ciclo AS c INNER JOIN ced_residentes AS r ON c.id_residente = r.id_residente")
    ...
    cursor.execute("SELECT * FROM ced_evaluacion WHERE grado NOT LIKE '' AND p_titular IS NOT NULL")
    evaluacion = cursor.fetchall()

    #Query for fixing residents IDs
    cursor.execute("SELECT e.id_empleados, r.matricula  FROM ced_evaluacion AS e INNER JOIN ced_residentes AS r ON e.id_empleados = r.id_residente")
    compare = cursor.fetchall()
    ...
    for eva in evaluacion:
        ...
        if pinvitado == 1:
            print(f"INSERT INTO ced_per_docente VALUES(DEFAULT, {id_eva}, 42, {fec_creacion}, now(), NULL)")
            cursor2.execute(f"INSERT INTO ced_per_docente VALUES(DEFAULT, {id_eva}, 42, {fec_creacion}, now(), NULL)")
